...
#' objective functions into a single \emph{utility function}. In the \pkg{spsann}-package, the aggregation 
...
#' 
#' A reasonable solution to avoid the numerical dominance of any objective funciton is to scale the objective
...
#' Using the \emph{upper-bound approach} requires the user to inform only the nadir point, while the utopia
...
#' exactly. If the user is uncomfortable with informing the nadir and utopia points, there is the option
#' for using \emph{numerical simulations}. It consists of computing the function value for many random system
#' configurations. The mean function value is used to set the nadir point, while the the utopia point is set 
#' to zero. This approach is similar to the upper-bound approach, but the function values will have the same
#' orders of magnitude only at the starting point of the optimization. Function values larger than one are 
#' likely to occur during the optimization. We recommend the user to avoid this approach whenever possible 
#' because the effect of the starting point on the optimization as a whole usually is insignificant or
#' arbitrary.
#' 
#' The \emph{upper-lower-bound approach} with the \emph{Pareto minimum and maximum values} is the most 
...
#' \item Record the minimum and maximum absolute function values computed for each objective function that
#' composes the MOCOP -- these are the Pareto minimum and maximum.
#' }
#' 
#' For example, consider that a MOCOP is composed of two objective functions: A and B. The minimum absolute
#' value for function A is obtained when the sample configuration is optimized with respect to function A. 
#' This is the Pareto minimum of function A. Consequently, the maximum absolute value for function A is 
#' obtained when the sample configuration is optimized regarding function B. This is the Pareto maximum of 
#' function A. The same logic applies for function B.
#'
#' @references
